...
const testController = {
    ...
    reset() {
        ...
        document.querySelector('.test-progress')?.remove();
    },
...
const statsController = {
    ...
    update() {
        ...
        const progress = input.value.length ? Math.min(100, (input.value.length / testController.data.originalText.length) * 100) : 0;
...
async function startTest() {
    try {
        ...
        // Show loading state
        if (DOM.startBtn) {
            const originalContent = DOM.startBtn.innerHTML;
            ...
            const cleanupLoading = () => {
                progressBar.remove();
                if (DOM.startBtn) {
                    DOM.startBtn.innerHTML = originalContent;
                }
            };
            ...
            try {
                ...
                if (DOM.paragraphDisplay) {
                    ...
                    testProgress.innerHTML = '<div class="test-progress-bar" style="width: 0%"></div>';
                    DOM.paragraphDisplay.parentNode.insertBefore(testProgress, DOM.paragraphDisplay.nextSibling);
                }
...
async function finishTest() {
    ...
    try {
        // Show loading state
        if (DOM.finishBtn) {
            DOM.finishBtn.innerHTML = '<span class="loading-spinner"></span>Submitting...';
        ...
        if (DOM.finishBtn) {
            DOM.finishBtn.innerHTML = 'Finish Test';
